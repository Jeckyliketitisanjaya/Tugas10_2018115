...
import com.example.tugas6_2018115.databinding.ActivityRegisterBinding;
...
public class RegisterActivity extends AppCompatActivity
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.SignUpbuttonid) {
            ...
            if (name.isEmpty()) {
                binding.signUpnameEditText.setError("Masukin Nama mu ");
                        binding.signUpnameEditText.requestFocus();
                return;
            }
            //checking the validity of the US
            if (username.isEmpty()) {
                binding.signUpusernameEditText.setError("Masukin username mu ");
                        binding.signUpusernameEditText.requestFocus();
                return;
            }
            //checking the validity of the Email
            if (email.isEmpty()) {
                binding.signiUpEmailEditText.setError("Masukin Email mu ");
                        binding.signiUpEmailEditText.requestFocus();
                return;
            }
            if (!Patterns.EMAIL_ADDRESS.matcher(email).matches())
            {
                binding.signiUpEmailEditText.setError("Ngisi itu yang bener ");
                        binding.signiUpEmailEditText.requestFocus();
                return;
            }
            //checking the validity of the password
            if (password.isEmpty()) {
                binding.signUppasswordEditText.setError("Masukin Passowrod ya ");
                        binding.signUppasswordEditText.requestFocus();
                return;
            }
            if (password.length() < 6) {
                binding.signUppasswordEditText.setError("Passowrd itu minimal 6");
                        binding.signUppasswordEditText.requestFocus();
                return;
            }
